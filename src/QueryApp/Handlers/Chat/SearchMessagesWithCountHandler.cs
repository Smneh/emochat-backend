...
public class SearchMessagesWithCountHandler : IRequestHandler<SearchMessagesWithCountQuery, List<GetMessagesResponseDto>>
{
    private readonly GroupRepository _groupRepository;
    private readonly IdentityService _identityService;
    private readonly ISender _sender;

    public SearchMessagesWithCountHandler(GroupRepository groupRepository, IdentityService identityService, ISender sender)
    {
    ...
    public async Task<List<GetMessagesResponseDto>> Handle(SearchMessagesWithCountQuery request, CancellationToken cancellationToken)
    {
        var result = await _groupRepository.GetGroupMessagesSearchResult(request.GroupId, request.MessageId, request.Count);

        // Handle IsSelf
        result.ForEach(message => message.IsSelf = _identityService.Username == message.RegUser);
    ...
    private async Task<List<Entities.Models.Profile.Profile>> _getProfileInfo(List<string> usernames)
    {
        ...
        return await _sender.Send(getProfilesByUsernamesRequest);
    }
}